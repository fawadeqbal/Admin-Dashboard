...
import Modal from '../modal/SensorModal'
// sections
import { SensorListHead, SensorListToolbar } from '../sections/@dashboard/sensor';
...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return filter(array, (_sensor) => _sensor.sensorId.toLowerCase().indexOf(query.toLowerCase()) !== -1);
  }
...
export default function SensorPage() {
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - sensorList.length) : 0;

  const filteredSensors = applySortFilter(sensorList, getComparator(order, orderBy), filterName);
    ...
    <>
      ...
      <Container>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          ...
          <Button variant="contained" startIcon={<Iconify icon="eva:plus-fill" />} onClick={() => setOpenCreateModal(true)}>
        New Sensor
      </Button>
        </Stack>
              ...
              <Table>
                ...
                <TableBody>
                  {filteredSensors.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((sensor) => {
                    ...
                    return (
                      <TableRow hover key={sensorId} tabIndex={-1} role="checkbox" selected={selectedSensor}>
                        <TableCell padding="checkbox">
                          <Checkbox checked={selectedSensor} onChange={(event) => handleClick(event, sensorId)} />
                        </TableCell>
                        ...
                        <TableCell align="right">
                          <IconButton size="large" color="inherit" onClick={(e)=>handleOpenMenu(e,sensor)}>
